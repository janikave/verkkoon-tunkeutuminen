...
![Add file: Upload](h2_nmap_vv.png)
...
![Add file: Upload](h2_log_nmap.png)
...
**21:00** Avasin kaksi komentoriviä, toinen porttiskannaukseen ja toinen wiresharkin avaamiseen. Katkaisin jälleen virtuaalikoneen nettiyhteyden ennen porttiskannausta

Avasin Wiresharkin ja nauhotin sillä toisella komentorivillä suorittamani porttiskannauksen. 

Tallensin nauhoituksen ja rupesin tarkastelemaan sitä ```frame contains "nmap"```-suodatuksella. 

![Add file: Upload](h2_ws.png) 

### f) Net grep[^1] 

**21:42**: Asensin ngrepin komennolla ```sudo apt-get install ngrep```.

Käytin komentoa ```sudo ngrep``` (ei onnistunut ilman sudoa) ja tein porttiskannauksen toisella rivillä (komento: ```sudo nmap -A -p 80 localhost```). 

Tuloksia, joissa on "nmap":

![Add file: Upload](h2_ngrep.png)
![Add file: Upload](h2_ngrep2.png)

### g) ja h)[^1]

**22:05**: Vaihdoin nmapin user-agentin lisäämällä komentoon vaihtoehdon ```--script-args http.useragent="Nokia 3110"```[^1]. 

Koko komento: ```sudo nmap -A -p 80 localhost --script-args http.useragent="Nokia 3110"```. 

Tämän jälkeen menin lokeihin ja katsoin viimeisimmät rivit: 

![Add file: Upload](h2_log_ua.png)

Näissä aiemmin näkynyt "Nmap Scripting Engine" on muuttunut komennon mukaisesti "Nokia 3110"-nimiseksi. 

### i) 

Yritin etsiä ```usr/share/nmap/```-hakemiston lua-tiedostosta, mutten lopulta löytänyt mitään, minkä muuttaminen kävisi itselleni järkeen. Etsin useilla eri avainsanoilla tiedostosta (esim. "nmap" ja "nse"), mutta en saanut tuloksista oikein mitään selvää. En myöskään löytänyt muista hakemiston tiedostoista kohtia, joita muuttamalla onnistuisin tässä. Tämä tehtävä siis epäonnistui.


## Lähteet 

Käytin tähän harjoitukseen seuraavia lähteitä:
